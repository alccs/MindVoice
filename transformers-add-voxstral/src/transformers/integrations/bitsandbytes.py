...
def _set_submodule_compat(model: nn.Module, target: str, new_module: nn.Module):
    """
    Compatible version of set_submodule for PyTorch < 2.5
    """
    if hasattr(model, 'set_submodule'):
        model.set_submodule(target, new_module)
    else:
        ...
        with torch.device("meta"):
            if isinstance(module, (nn.Linear, Conv1D)):
                ...
                if new_module is not None:
                    ...
                    _set_submodule_compat(model, module_name, new_module)
